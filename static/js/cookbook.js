...
async function _fetchDependencies() {
  ...
  try {
    ...
    function _showDepMenu(anchor) {
      ...
      // DOMRect/window values are rendered pixels under the Larger text-size
      // CSS zoom; fixed inline lengths are authored pixels.
      ...
      dropdown.style.cssText = `position:fixed;display:block;z-index:${topPortalZ()};top:${(rect.bottom + 6) / zr}px;left:${left / zr}px;right:auto;min-width:${150}px;max-width:calc(100vw - ${16 / zr}px);background:var(--panel,var(--bg));border:1px solid var(--border);border-radius:10px;box-shadow:0 8px 24px rgba(0,0,0,0.3);padding:6px;font-size:11px;`;
